...
import { ColorModel, Harmony, Mix, MIN_DECIMALS } from '#constants';
...
export class ColorTranslator {
    ...
    public static getBlendHEX(
        from: ColorInput,
        to: ColorInput,
        steps: number = defaultBlendSteps,
        css = true
    ): HEXOutput[] {
    ...
    public static getHarmony(
        ...
        switch (armony) {
            case Harmony.ANALOGOUS:
                return utils.colorHarmony.buildHarmony(
                    color,
                    utils.analogous,
                    mode
                );
