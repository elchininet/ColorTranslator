...
//---Normalize alpha
export const normalizeAlpha = (
    alpha: number | string | undefined | null
): number => {
            ...
            ...arr,
            {
                ...
                h:
                    mode === Mix.ADDITIVE
                        ...
                        : normalizeHue(
                              hueRYB(hueRYB(color.h, false) + num, true)
                          )
            }
        ],
        [{ ...color }]
...
export const analogous = (color: HSLObject, mode: Mix): HSLObject[] =>
    harmony(color, [30, -30], mode);
export const complementary = (color: HSLObject, mode: Mix): HSLObject[] =>
    harmony(color, [180], mode);
export const splitComplementary = (color: HSLObject, mode: Mix): HSLObject[] =>
    harmony(color, [150, -150], mode);
export const triadic = (color: HSLObject, mode: Mix): HSLObject[] =>
    harmony(color, [120, -120], mode);
export const tetradic = (color: HSLObject, mode: Mix): HSLObject[] =>
    harmony(color, [60, -120, 180], mode);
export const square = (color: HSLObject, mode: Mix): HSLObject[] =>
    harmony(color, [90, -90, 180], mode);
...
const VALID_COLOR_OBJECTS = Object.entries(ColorModel).reduce(
    (acc: Record<string, ColorModel>, entry: [string, ColorModel]) => {
...
const getColorModelFromObject = (color: Color): ColorModel => {
    ...
    if (model && model === ColorModel.RGB) {
        const hasInvalidHex = Object.entries(color).some(
            (item: [string, string | number]): boolean => {
                return !HEX.test(`${item[1]}`);
            }
        );

        const hasInvalidRegb = Object.entries(color).some(
            (item: [string, string | number]): boolean => {
                return !(
                    PCENT.test(`${item[1]}`) ||
                    (!HEX.test(`${item[1]}`) &&
...
export const getRGBObject = (
    color: ColorInput,
    model: ColorModel = getColorModel(color)
): RGBObject => {
    return typeof color === 'string'
        ? getRGBObjectFromString[model](color)
        : getRGBObjectFromObject[model](
              color as RGBObjectGeneric & HSLObjectGeneric & CMYKObjectGeneric
          );
};
...
export const translateColor = {
    ...
    RGBA(color: RGBObject): RGBObject {
        color.a = hasProp<RGBObject>(color, 'a') ? round(color.a, 2) : 1;
        return color;
    },
    ...
    HSLA(color: RGBObject): HSLObject {
        ...
        hsl.a = hasProp<RGBObject>(color, 'a') ? round(color.a, 2) : 1;
        return hsl;
    },
    ...
    CMYKA(color: RGBObject): CMYKObject {
        ...
        cmyk.a = hasProp<RGBObject>(color, 'a') ? round(color.a, 2) : 1;
        return cmyk;
    }
};
...
export const getColorMixture = (
    ...
    const hasAlpha =
        ...
        (typeof color !== 'string' &&
            hasProp<RGBObjectGeneric | HSLObjectGeneric>(color, 'a'));
    ...
    switch (model) {
        ...
        default:
            return hslMap.map((hslColor: HSLObject): HEXOutput => {
                ...
                if (isCSS) {
                    if (hasAlpha)
                        rgbColor.a = round(rgbColor.a * 255, MAX_DECIMALS);
                    return CSS.HEX(rgbColor);
                } else {
                    return hasAlpha
        ...
        function createMix(items: RGYBObject[]): RGYBObject {
            const initial =
                mode === Mix.ADDITIVE
                    ...
                    : { r: 0, y: 0, b: 0, a: 0 };
            return items.reduce(
                (mix: RGYBObject, color: RGYBObject): RGYBObject => {
                    const colorA = hasProp<RGYBObject>(color, 'a')
                        ? color.a
                        : 1;
                    const common = {
