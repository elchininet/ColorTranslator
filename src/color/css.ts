...
import { ColorModel, TEMPLATE_VAR, COLOR_PROPS, VALID_COLOR_OBJECTS } from '#constants';
import {
    toHEX,
    round,
    percent,
...
const prepareColorForCss = (color: Color, transformer: Transformer): NumberOrString[] => {
    const props = getOrderedArrayString(Object.keys(color));
    const model = VALID_COLOR_OBJECTS[props];
    const keys = COLOR_PROPS[model];
    return keys.reduce(
        (result: NumberOrString[], key: keyof typeof color, index: number): NumberOrString[] => {
...
const getAlpha = (value: number, options: Options, ignoreLegacy = false): NumberOrString => {
    ...
    if (alphaUnit === ColorUnitEnum.PERCENT && (!legacyCSS || ignoreLegacy)) {
...
export const CSS = {
    [ColorModel.HEX]: (color: HEXObject | RGBObject): string => {
        ...
        const template = values.length === 4 ? '#{1}{2}{3}{4}' : '#{1}{2}{3}';
    ...
    [ColorModel.HSL]: (color: HSLObject, options: Options): string => {
        ...
        const comma = getComma(spacesAfterCommas);
        const transformer = (value: number, index: number): NumberOrString => {
            if (index === 0 && anglesUnit !== AnglesUnitEnum.NONE) {
                const translated = round(translateDegrees(value, anglesUnit), decimals);
                return `${translated}${anglesUnit}`;
            }
            return index === 3 ? getAlpha(value, options) : round(value, decimals);
        };
        const values = prepareColorForCss(color, transformer);
        const template = legacyCSS
            ? values.length === 4
    ...
    [ColorModel.CIELab]: (color: CIELabObject, options: Options) => {
        ...
        const transformer = (value: number, index: number): NumberOrString => {
            if (index === 0) {
                ...
                return labUnit === ColorUnitEnum.PERCENT ? `${L}%` : `${L}`;
            }
            if (index < 3) {
        ...
        const values = prepareColorForCss(color, transformer);
        const template = values.length === 4 ? `lab({1} {2} {3} / {4})` : `lab({1} {2} {3})`;
        return getResultFromTemplate(template, values);
    },
    [ColorModel.CMYK]: (color: CMYKObject, options: Options): string => {
        ...
        const comma = getComma(spacesAfterCommas);
        const transformer = (value: number, index: number): NumberOrString => {
            if (cmykUnit === ColorUnitEnum.PERCENT && index < 4) {
                return `${round(value, decimals)}%`;
            }
            return index === 4 ? getAlpha(value, options) : round(value / 100, decimals);
        };
        const values = prepareColorForCss(color, transformer);
        const template = legacyCSS
            ? values.length === 5
