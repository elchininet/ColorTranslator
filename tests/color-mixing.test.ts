...
describe('Additive Color mixing', (): void => {
    mixFunctions.forEach((fnObject): void => {
        ADDITIVE_MIXES.forEach((item: MixObject): void => {
            ...
            it(`Regular CSS additive mix using ${
                fnObject.name
            } ${JSON.stringify(colors)} => ${mix}`, (): void => {
                expect(mix).toBe(fnObject.fn(item.mix));
            });
        });
        ADDITIVE_MIXES.forEach((item: MixObject): void => {
            const colors = item.colors.map(
                (c): RGBOutput | HSLOutput | HEXOutput => fnObject.fn(c, false)
            );
            const mix = fnObject.mixFn(colors, Mix.ADDITIVE, false);
            it(`Regular Object additive mix using ${
                fnObject.name
            } ${JSON.stringify(colors)} => ${JSON.stringify(
                mix
            )}`, (): void => {
...
describe('Subtractive Color mixing', (): void => {
    ...
    SUBTRACTIVE_MIXES.forEach((item: MixObject): void => {
        const colors = item.colors.map((c): string =>
            ColorTranslator.toHEXA(c)
        );
        const mix = ColorTranslator.getMixHEXA(colors, Mix.SUBTRACTIVE);
        it(`Regular CSS subtractive mix using getMixHEXA ${JSON.stringify(
            colors
        )} => ${mix}`, (): void => {
...
describe('Strong additive colors mixing', (): void => {
    it('Black with blue', (): void => {
        expect(
            ColorTranslator.getMixHEX(['#000000', '#0000FF'], Mix.SUBTRACTIVE)
        ).toBe('#000000');
    });
    it('Black with White', (): void => {
        expect(
            ColorTranslator.getMixHEX(['#000000', '#FFFFFF'], Mix.SUBTRACTIVE)
        ).toBe('#000000');
    });
});
