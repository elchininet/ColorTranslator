...
const mixFunctions = [
    {
        name: 'getMixHEX',
        mixFn: ColorTranslator.getMixHEX,
        fn: ColorTranslator.toHEX
    },
    {
        name: 'getMixHEXA',
        mixFn: ColorTranslator.getMixHEXA,
        fn: ColorTranslator.toHEXA
    },
    {
        name: 'getMixRGB',
        mixFn: ColorTranslator.getMixRGB,
        fn: ColorTranslator.toRGB
    },
    {
        name: 'getMixRGBA',
        mixFn: ColorTranslator.getMixRGBA,
        fn: ColorTranslator.toRGBA
    },
    {
        name: 'getMixHSL',
        mixFn: ColorTranslator.getMixHSL,
        fn: ColorTranslator.toHSL
    },
    {
        name: 'getMixHSLA',
        mixFn: ColorTranslator.getMixHSLA,
        fn: ColorTranslator.toHSLA
    },
    {
        name: 'getMixHEXObject',
        mixFn: ColorTranslator.getMixHEXObject,
        fn: ColorTranslator.toHEXObject
    },
    {
        name: 'getMixHEXAObject',
        mixFn: ColorTranslator.getMixHEXAObject,
        fn: ColorTranslator.toHEXAObject
    },
    {
        name: 'getMixRGBObject',
        mixFn: ColorTranslator.getMixRGBObject,
        fn: ColorTranslator.toRGBObject
    },
    {
        name: 'getMixRGBAObject',
        mixFn: ColorTranslator.getMixRGBAObject,
        fn: ColorTranslator.toRGBAObject
    },
    {
        name: 'getMixHSLObject',
        mixFn: ColorTranslator.getMixHSLObject,
        fn: ColorTranslator.toHSLObject
    },
    {
        name: 'getMixHSLAObject',
        mixFn: ColorTranslator.getMixHSLAObject,
        fn: ColorTranslator.toHSLAObject
    }
];
...
describe('Additive Color mixing', (): void => {
    mixFunctions.forEach((fnObject): void => {
        ADDITIVE_MIXES.forEach((item: MixObject): void => {
            ...
            it(`Regular additive mix using ${fnObject.name} ${JSON.stringify(colors)} => ${JSON.stringify(
                mix
            )}`, (): void => {
...
describe('Subtractive Color mixing', (): void => {
    SUBTRACTIVE_MIXES.forEach((item: MixObject): void => {
        ...
        it(`Regular CSS subtractive mix using getMixHEX ${JSON.stringify(
            item.colors
        )} => ${mix}`, (): void => {
...
describe('Color mixing with alphas', (): void => {
    ADDITIVE_MIXES.forEach((item: MixObject): void => {
        ...
        it(`Regular CSS additive mix with alpha using getMixHEXA ${JSON.stringify(
            colors
        )} => ${mix}`, (): void => {
            if (colors.length === 3) {
    ...
    SUBTRACTIVE_MIXES.forEach((item: MixObject): void => {
        ...
        const mix = ColorTranslator.getMixHEXA(colors);
        it(`Regular CSS subtractive mix with alpha using getMixHEXA ${JSON.stringify(
            colors
        )} => ${mix}`, (): void => {
            if (colors.length === 3) {
                expect(mix.slice(-2)).toBe('B3');
...
describe('Color mixing with L*a*b colors', (): void => {
    ...
    it('Return a mix in lab color with decimals', (): void => {
        expect(ColorTranslator.getMixCIELab(['#F00', '#00F'])).toBe('lab(60.169696 93.550025 -60.498556)');
    });
    ...
    it('Return a mix in lab color as an object with decimals', (): void => {
        ...
        ).toMatchObject({ L: 60.169696, a: 93.550025, b: -60.498556 });
    });
    it('Return a substractive mix in lab color as an object with decimals', (): void => {
        ...
        ).toMatchObject({ L: 39.282789, a: 74.659748, b: -95.543967 });
    });
    it('Return a mix in lab color with alpha', (): void => {
        expect(
            ColorTranslator.getMixCIELabA(['#F00', '#00F'], Mix.ADDITIVE, {
                decimals: 0
            })
        ).toBe('lab(60 94 -60 / 1)');
    });
    it('Return a substractive mix in lab color with alpha', (): void => {
        expect(ColorTranslator.getMixCIELabA(['#F00', '#00F'], Mix.SUBTRACTIVE)).toBe(
            'lab(39.282789 74.659748 -95.543967 / 1)'
        );
    });
    it('Return a mix in lab color with alpha and decimals', (): void => {
        expect(ColorTranslator.getMixCIELabA(['#F00', '#00F'])).toBe(
    ...
    it('Return a mix in lab color with alpha as an object', (): void => {
        ...
        ).toMatchObject({ L: 60, a: 94, b: -60, A: 1 });
    });
    it('Return a substractive mix in lab color with alpha as an object with decimals', (): void => {
        ...
        ).toMatchObject({ L: 39.282789, a: 74.659748, b: -95.543967, A: 1 });
    });
    it('Return a mix in lab color with alpha as an object with decimals', (): void => {
        ...
        ).toMatchObject({ L: 60.169696, a: 93.550025, b: -60.498556, A: 1 });
    });
});
...
describe('rgbUnit auto detection', (): void => {
    ...
    it('All rgb colors with rgb units, output should be with rgb units', () => {
        expect(ColorTranslator.getMixRGB(['rgb(100% 100% 100%)', 'rgb(100% 100% 100%)'])).toBe(
            'rgb(100% 100% 100%)'
        );
    });
    it('Rgb units mixed, output should be the default: none', () => {
        expect(ColorTranslator.getMixRGB(['rgb(100% 100% 100%)', 'rgb(255 255 255)'])).toBe(
            'rgb(255 255 255)'
        );
    });
});
