...
COLORS.forEach((item): void => {
    describe(`CIE L*a*b tests for ${item.HEX}`, (): void => {
        ...
        it('legacyCSS should be ignored', () => {
            const lab = ColorTranslator.toCIELab(item.HEX);
            const labLegacy = ColorTranslator.toCIELab(item.HEX, {
                legacyCSS: true
            });
            const labLegacyWithAlphaInPercent = ColorTranslator.toCIELabA(item.HEX, {
                legacyCSS: true,
                alphaUnit: 'percent'
            });
...
describe('L*a*b with percentages', (): void => {
    it('L in percentages should be the same as a number', () => {
        ...
        Lvalues.forEach((L: number): void => {
            ...
            expect(ColorTranslator.toHEX(`lab(${L} ${x} ${x})`)).toBe(
    ...
    it('a or b in percentages should be the same as a number', () => {
        ...
        values.forEach((v): void => {
            ...
            expect(ColorTranslator.toHEX(`lab(${L}% ${v[0]} ${x})`)).toBe(
                ColorTranslator.toHEX(`lab(${L}% ${v[1]} ${x})`)
            );

            expect(ColorTranslator.toHEX(`lab(${L}% ${x} ${v[0]})`)).toBe(
...
describe('L*a*b with alpha', (): void => {
    it('Alpha by default should be 1 if not specified', () => {
        expect(ColorTranslator.toCIELabA('rgb(255 0 0)', { decimals: 1 }).endsWith('/ 1)')).toBe(true);
        expect(ColorTranslator.toCIELabAObject('rgb(255 0 0)', { decimals: 1 })).toHaveProperty('A', 1);
    });

    it('Alpha with a number should be maintained', () => {
        expect(
            ColorTranslator.toCIELabA('rgb(255 0 0 / 0.5)', {
                decimals: 1
            }).endsWith('/ 0.5)')
        ).toBe(true);
        expect(
            ColorTranslator.toCIELabAObject('rgb(255 0 0 / 0.5)', {
                decimals: 1
            })
        ).toHaveProperty('A', 0.5);
    });

    it('Alpha with a percentage should be maintained', () => {
        expect(
            ColorTranslator.toCIELabA('rgb(255 0 0 / 50%)', {
                decimals: 1
            }).endsWith('/ 50%)')
        ).toBe(true);
        expect(
            ColorTranslator.toCIELabAObject('rgb(255 0 0 / 50%)', {
                decimals: 1
            })
        ).toHaveProperty('A', 0.5);
    });
    ...
    it('Alpha should not be present in non-alpha methods', () => {
        expect(
            ColorTranslator.toCIELab('rgb(255 0 0 / 1)', {
                decimals: 1
            }).endsWith('/ 1)')
        ).toBe(false);
        expect(
            ColorTranslator.toCIELabObject('rgb(255 0 0 / 0.5)', {
                decimals: 1
            })
