...
COLORS.forEach((color): void => {
    Object.values(color).forEach(colorValue => {
        ...
        describe(`ColorTranslator static color conversion for ${colorValueStr}`, () => {
            ...
            it(`toHEX method as Object from ${colorValueStr} => ${JSON.stringify(
                color.hexObject
            )}`, () => {
                expect(ColorTranslator.toHEX(colorValue, false)).toMatchObject(
                    color.hexObject
                );
            });
            ...
            it(`toHEXA method as Object from ${colorValueStr} => ${JSON.stringify(
                color.hexaObject
            )}`, () => {
                expect(ColorTranslator.toHEXA(colorValue, false)).toMatchObject(
                    color.hexaObject
                );
            });
            ...
            it(`toRGB method as Object from ${colorValueStr} => ${JSON.stringify(
                color.rgbObject
            )}`, () => {
                expect(ColorTranslator.toRGB(colorValue, false)).toMatchObject(
                    color.rgbObject
                );
            });
            ...
            it(`toRGBA method as Object from ${colorValueStr} => ${JSON.stringify(
                color.rgbaObject
            )}`, () => {
                expect(ColorTranslator.toRGBA(colorValue, false)).toMatchObject(
                    color.rgbaObject
                );
            });
            ...
            it(`toHSL method as Object from ${colorValueStr} => ${JSON.stringify(
                color.hslObject
            )}`, () => {
                expect(ColorTranslator.toHSL(colorValue, false)).toMatchObject(
                    color.hslObject
                );
            });
            ...
            it(`toHSLA method as Object from ${colorValueStr} => ${JSON.stringify(
                color.hslaObject
            )}`, () => {
                expect(ColorTranslator.toHSLA(colorValue, false)).toMatchObject(
                    color.hslaObject
                );
            });
        });
...
CMYK_COLORS.forEach(color => {
    Object.values(color).forEach(colorValue => {
        ...
        describe(`ColorTranslator static CMYK color conversion for ${colorValueStr}`, () => {
            ...
            it(`toCMYK method as Object from ${colorValueStr} => ${JSON.stringify(
                color.cmykIntObject100
            )}`, () => {
                expect(ColorTranslator.toCMYK(colorValue, false)).toMatchObject(
                    color.cmykIntObject100
                );
            });
            ...
            it(`toCMYKA method as CSS from ${colorValueStr} => ${color.cmykWithAlpha}`, () => {
                expect(ColorTranslator.toCMYKA(colorValue)).toBe(
                    color.cmykWithAlpha
                );
            });

            it(`toCMYKA method as Object from ${colorValueStr} => ${JSON.stringify(
                color.cmykIntObject100WithAlpha
            )}`, () => {
                expect(
                    ColorTranslator.toCMYKA(colorValue, false)
                ).toMatchObject(color.cmykIntObject100WithAlpha);
            });
        });
    });
});
