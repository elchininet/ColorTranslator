...
describe('ColorTranslator shades and tints tests', (): void => {
    ...
    colorFunctions.forEach((fn): void => {
        ...
        const inputObject = fn(base, false) as HEXObject &
            RGBObject &
            HSLObjectGeneric;
        const alpha = '80';
        const inputCSSWithAlpha = fn(base + alpha);
        const inputObjectWithAlpha = fn(base + alpha, false) as HEXObject &
            RGBObject &
            HSLObjectGeneric;
        ...
        it(`Shades tests from ${JSON.stringify(inputObject)}`, (): void => {
            const output = shades_results.map((color: string) =>
                fn(color, false)
            );
            const shades = ColorTranslator.getShades(inputObject, 5) as (
                | HEXObject
                | RGBObject
                | HSLObjectGeneric
            )[];
        ...
        it(`Shades tests with alpha from ${JSON.stringify(
            inputCSSWithAlpha
        )}`, (): void => {
            const output = shades_results.map((color: string) =>
                fn(color + alpha)
        ...
        it(`Shades tests with alpha from ${JSON.stringify(
            inputObjectWithAlpha
        )}`, (): void => {
            const output = shades_results.map((color: string) =>
                fn(color + alpha, false)
            );
            const shades = ColorTranslator.getShades(
                inputObjectWithAlpha,
                5
        ...
        it(`Tints tests from ${JSON.stringify(inputObject)}`, (): void => {
            const output = tints_results.map((color: string) =>
                fn(color, false)
            );
            const shades = ColorTranslator.getTints(inputObject, 5) as (
                | HEXObject
                | RGBObject
                | HSLObjectGeneric
            )[];
        ...
        it(`Tints tests with alpha from ${JSON.stringify(
            inputCSSWithAlpha
        )}`, (): void => {
            const output = tints_results.map((color: string) =>
                fn(color + alpha)
        ...
        it(`Tints tests with alpha from ${JSON.stringify(
            ...
            const shades = ColorTranslator.getTints(
                inputObjectWithAlpha,
                5
            ) as (HEXObject | RGBObject | HSLObjectGeneric)[];
            expect(shades).toMatchObject(output);
